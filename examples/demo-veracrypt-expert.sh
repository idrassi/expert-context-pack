...
python -m ecp_poc.cli validate --with-artifacts --skill "${skill}" --json       
...
if [[ "${skill}" == "examples/veracrypt-expert-hybrid-vector" ]]; then
  ...
  python -c "import json; from ecp_poc.vectors import hash_embed; print(json.dumps(hash_embed('Where is Argon2 implemented?', dims=256, salt='veracrypt-hash-embed-v1', include_char_ngrams=True, char_ngram=3, char_ngram_weight=0.5), ensure_ascii=False))" > "${qvec_file}"
  python -m ecp_poc.cli query --skill "${skill}" --query-vector-file "${qvec_file}" --source-id repo "Where is Argon2 implemented?"
fi
