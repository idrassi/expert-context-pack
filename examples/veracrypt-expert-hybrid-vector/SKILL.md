...
description: Repository-aware expert for VeraCrypt using hybrid retrieval (vector + SQLite FTS) with incremental refresh and optional OpenRouter LLM synthesis.
license: Apache-2.0
...
# VeraCrypt Expert (Hybrid: Vector + SQLite FTS, ECP v1.0)
...
This skill supports hybrid retrieval:
- Lexical: SQLite FTS (`fts-v1`)
- Vector: sparse vector index (`vec-v1`)

To provide an external query embedding (dense `[float,...]` or sparse `[[i,v],...]`):

- `ecpctl query --json --skill <skill_dir> --query-vector-file /path/to/query_vector.json "<question>"`
