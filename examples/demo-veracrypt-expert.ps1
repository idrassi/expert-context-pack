...
python -m ecp_poc.cli validate --with-artifacts --skill $skill --json | Out-Host
python -m ecp_poc.cli query --skill $skill --source-id repo "Where are the license terms located?"
python -m ecp_poc.cli query --skill $skill --source-id repo "Where is Argon2 implemented?"
if ($skill -eq "examples/veracrypt-expert-hybrid-vector") {
  $qvec = Join-Path $outDir "query_vector.json"
  python -c "import json; from ecp_poc.vectors import hash_embed; print(json.dumps(hash_embed('Where is Argon2 implemented?', dims=256, salt='veracrypt-hash-embed-v1', include_char_ngrams=True, char_ngram=3, char_ngram_weight=0.5), ensure_ascii=False))" | Out-File -Encoding utf8 $qvec
  python -m ecp_poc.cli query --skill $skill --query-vector-file $qvec --source-id repo "Where is Argon2 implemented?"
}
...
if ($hasOpenRouterKey) {
  python -m ecp_poc.cli query --llm openrouter --skill $skill "Summarize the main components involved in mounting a volume."
}
