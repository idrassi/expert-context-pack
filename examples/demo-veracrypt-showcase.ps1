...
if ($skill -eq "examples/veracrypt-expert-hybrid-vector") {
    ...
    python -c 'import json; from ecp_poc.vectors import hash_embed; print(json.dumps(hash_embed("Where is Argon2 key derivation implemented?", dims=256, salt="veracrypt-hash-embed-v1", include_char_ngrams=True, char_ngram=3, char_ngram_weight=0.5), ensure_ascii=False))' | Out-File -Encoding utf8 $qvec
